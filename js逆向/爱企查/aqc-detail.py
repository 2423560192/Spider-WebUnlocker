...
def get_company_pid():
    with open('data.json', 'r', encoding='utf-8') as f:
        datas = json.loads(f.read())[0]
    return datas


# 获取数据
def get_company_data(pid):
    url = "https://aiqicha.baidu.com/detail/basicAllDataAjax?pid=30146585261724"
    payload = {
        'pid': pid
    }

    resp = requests.get(url, headers=headers, params=payload , verify=False)

    if resp.status_code == 200:
        data = resp.json()
        print(data)
        # 获取企业数据
        name = jsonpath(data, '$..data.basicData.entName')[0]  # 企业名
        regNo = jsonpath(data, '$..data.basicData.regNo')[0]  # 社会信用代码
...
if __name__ == '__main__':
    datas = get_company_pid()

    # 开始获取企业数据
    for name , pid in datas.items():
        print('pid' , pid)
