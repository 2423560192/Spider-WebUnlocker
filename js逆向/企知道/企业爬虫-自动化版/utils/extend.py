...
async def login(username, password):
    ...
    await page.waitForXPath(
        '//*[@id="__layout"]/div/div[4]/div[2]/div/div/div[1]/div/div/div[2]/div/div[1]/div[2]/span')
    ...
    # 等待登录成功后再跳转
    await page.waitForSelector('span.companyName', {'timeout': 60000})  # 等待某个登录后的页面元素
    print("登录成功")
...
async def get_company_info(url, page):
    """
    ...
    names = lxml_tree.xpath('//span[@class="companyName"]//text()')[:10]
    ids = lxml_tree.xpath('//span[@class="credit-code"]//text()')[:10]
    urls = lxml_tree.xpath('//div[@class="company-card-wrapper"]/div[@class="imgs-box"]/a/@href')[:10]
    ...
    print('--------------------------------')
    lst = []
    for name , id , url in zip(names , ids , urls):
        dic = {
            'company_name':name,
            'company_id':id,
            'url':url,
