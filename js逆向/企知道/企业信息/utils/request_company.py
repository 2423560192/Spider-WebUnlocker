...
from .extends import change_ip_cookie, get_ua
from .handle_data import parse_butie_data
...
def get_company_detail_data(url, proxies, cookie, ua):
    """
    抓取企业详情信息
    :param url:
    :return:
...
def get_company_detail_butie(name, id, proxy, cookie, ua):
    headers = {
    ...
    response = requests.post(
        'https://app.qizhidao.com/qzd-bff-marketing/library/search/searchNew',
        headers=headers,
        json=json_data,
        proxies=proxy
    )
    ...
    print(data)
    # 解析数据
    butie_data = parse_butie_data(data, id)

    total_page = int(jsonpath(data, '$..data.total')[0])

    if len(butie_data) == total_page:
        return butie_data

    # 剩余的
    # 计算循环次数
    num = total_page // 100 + 1
    ...
    for i in range(2, num + 1):
        ...
        data = response.json()
        butie_data.extend(parse_butie_data(data, id))
