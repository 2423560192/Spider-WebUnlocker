...
class GoodsApp:
    def __init__(self, root):
        self.stop_thread_flag = False  # 添加线程停止标志变量并初始化为False
        self.root = root
    ...
    def _create_top_frame(self):
        ...
        # 加载并显示Logo图像
        try:
            logo_img = Image.open("utils/logo.png")  # 替换为你的 logo 文件路径
    ...
    def _create_dft_page(self):
        ...
        # 读取 1.txt 文件内容
        try:
            with open('utils/1.txt', 'r', encoding='utf-8') as file:
                md_text = file.read()
        except Exception as e:
        ...
        # 打开并调整图片大小
        try:
            img = Image.open('utils/huaxue.png')  # 请确保 huaxue.png 文件存在
    ...
    def _create_md_page(self):
        ...
        canvas.create_window((0, 0), window=content_frame, anchor="nw")
        # 打开并调整图片大小
        img = Image.open('utils/zhengfangti.png')  # 请确保 huaxue.png 文件存在
    ...
    def _search_goods(self):
        """根据用户输入进行普通搜索。"""
        self.stop_thread_flag = True  # 设置停止标志为True，通知正在执行的线程停止
    ...
    def _perform_search(self, query):
        ...
        try:
            ...
            while True:  # 添加循环用于定期检查停止标志
                if self.stop_thread_flag:  # 检查停止标志是否被设置为True
                    break  # 如果是，跳出循环，结束线程执行
                ...
                break  # 如果正常执行完一轮查询且未被停止标志中断，也跳出循环
