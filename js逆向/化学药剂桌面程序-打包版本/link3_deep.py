...
def detail_data(sku , cookies , name ,cas , molecular_weight):
    end_lst = []
    try:
        purity = "".join(map(str, sku.xpath('.//p//text()')))
    except:
        purity = ''

    try:
        sku = sku.xpath('./h3//text()')[0]
        # 请求sku
...
def get_data( cp, url):
    ...
    for info in infos:
        ...
        with ThreadPoolExecutor(max_workers=3) as pool:
            for sku in skus:
                end_lst.append(pool.submit(detail_data , sku , cookies , name,csa , molecular_weight))

        for i in end_lst:
            try:
                res_lst.extend(i.result())
            except:
                pass
        # print(res_lst)
        # break

    save_data(conn, res_lst)
    conn.close()


def link3_deep_main(name):
    USER_AGENTS = [
    ...
    # 获取页数
    cp.get(url)
    page = cp.ele('xpath://*[@id="main-content"]/div[1]/div/div/div/div[2]/div[3]/div[2]/div/div[1]').text
    ...
    try:
        for i in range(1, page + 1):
            cp = ChromiumPage(co)
            url = f'https://www.sigmaaldrich.cn/CN/zh/search/{name}?focus=products&page={i}&perpage=30&sort=relevance&term={name}&type=product'
            # 获取数据
            get_data(cp, url)
            # 关闭
            cp.close()
