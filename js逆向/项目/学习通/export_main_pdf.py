import requests
from lxml import etree
from jinja2 import Template
import pdfkit

headers = {
    "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
    "Accept-Language": "zh-CN,zh;q=0.9",
    "Cache-Control": "no-cache",
    "Connection": "keep-alive",
    "Pragma": "no-cache",
    "Sec-Fetch-Dest": "document",
    "Sec-Fetch-Mode": "navigate",
    "Sec-Fetch-Site": "none",
    "Sec-Fetch-User": "?1",
    "Upgrade-Insecure-Requests": "1",
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/137.0.0.0 Safari/537.36",
    "sec-ch-ua": "\"Google Chrome\";v=\"137\", \"Chromium\";v=\"137\", \"Not/A)Brand\";v=\"24\"",
    "sec-ch-ua-mobile": "?0",
    "sec-ch-ua-platform": "\"Windows\""
}
cookies = {
    "fid": "10948",
    "k8s": "1750044485.218.18710.522447",
    "route": "2fe558bdb0a1aea656e6ca70ad0cad20",
    "jrose": "D7108BD6C96B461BE95552B387B9D46A.mooc-2281852096-lfnh5",
    "fanyamoocs": "11401F839C536D9E",
    "_uid": "350880339",
    "_d": "1750061424312",
    "UID": "350880339",
    "vc3": "UOxWdYo4wyNq85gsFu8iUyuvunQkKXrTGojZdhqDoF2kt%2BFiqZ%2F0lJn%2BalGtBY4ys1WP7r7%2BlNf2JoF3G76eho8IhR6awMPN6hNcdvaSksmpuHYOjsXcoqxUYTPmEQDdSc%2FE3a4hxk2mL%2Fcb7cQVg6JRaD8hHRg2XAx7%2BF%2FxqUE%3D2246faf87a8344e740926bc20d0b81ea",
    "uf": "",
    "cx_p_token": "e69e0526d0e74d7dcbf8a95be431b4d2",
    "p_auth_token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1aWQiOiIzNTA4ODAzMzkiLCJsb2dpblRpbWUiOjE3NTAwNjE0MjQzMTMsImV4cCI6MTc1MDY2NjIyNH0.8gHADhDTt_dq8eqxnzJaxaXvwnDnMiIdcD4cKJNeTas",
    "xxtenc": "543b24dd06366c61c47c7c02ca589564",
    "DSSTASH_LOG": "C_38-UN_10404-US_350880339-T_1750061424313",
    "source": "\"\"",
    "spaceFid": "10948",
    "spaceRoleId": "\"\"",
    "tl": "1"
}
params = {
    "courseId": "253237693",
    "classId": "122999526",
    "cpi": "416702459",
    "workId": "44072813",
    "answerId": "54702781",
    "standardEnc": "87a0db588c39cdf051074133cfb2a0ce",
    "enc": "cf1319545f9bdb39bf308e95f306e77c"
}

response = requests.get('https://mooc1.chaoxing.com/mooc-ans/mooc2/work/dowork', params=params,
                        cookies=cookies, headers=headers)
html = etree.HTML(response.text)

ques_list = html.xpath('//div[@class="padBom50 questionLi fontLabel singleQuesId"]')
ans_list = html.xpath('//div[contains(@class, "stem_answer")]')

export_data = []


def process(ques, ans):
    try:
        q_text = " ".join([
            i.strip() for i in
            ques.xpath('./h3/text() | ./h3/following-sibling::p[position()<=2]/text() | ./h3/span/text()')
        ])
        dic = {}
        if '简答题' in q_text or '论述题' in q_text:
            textarea = ans.xpath('./div[@class="eidtDiv"]/textarea/text()')
            answer = textarea[0].strip() if textarea else "暂无答案"
            export_data.append({"question": q_text, "options": {}, "answer": [answer]})
        elif '材料题' in q_text:
            textarea = ans.xpath('.//div[@class="textDIV"]/textarea/text()')
            answer = textarea[0].strip() if textarea else "暂无答案"
            export_data.append({"question": q_text, "options": {}, "answer": [answer]})
        else:
            options = ans.xpath('./div[@class="clearfix answerBg workTextWrap"]')
            correct = []
            for opt in options:
                key = opt.xpath('./span/text()')[0]
                val = opt.xpath('./div/p/text()')[0]
                if opt.xpath('./span[contains(@class, "check_answer")]'):
                    dic[key] = val + '    checked'
                    correct.append(key)
                else:
                    dic[key] = val
            export_data.append({"question": q_text, "options": dic, "answer": correct})
    except Exception as e:
        print("错误处理题目：", e)


for q, a in zip(ques_list, ans_list):
    process(q, a)

# Jinja2 HTML模板
html_template = """
<!DOCTYPE html>
<html lang="zh">
<head>
    <meta charset="UTF-8">
    <style>
        body { font-family: "SimSun", serif; padding: 30px; line-height: 1.6; }
        h1 { text-align: center; color: #333; }
        .question { margin-bottom: 30px; }
        .q-title { font-weight: bold; color: #222; margin-bottom: 10px; }
        .option { margin-left: 20px; }
        .checked { color: green; font-weight: bold; }
    </style>
</head>
<body>
    <h1>{{ title }}</h1>
    {% for item in data %}
    <div class="question">
        <div class="q-title"> {{ item.question }}</div>
        {% if item.options %}
        <ul>
            {% for key, val in item.options.items() %}
            <li class="option {% if 'checked' in val %}checked{% endif %}">
                {{ key }}. {{ val.replace('checked', '').strip() }}
            </li>
            {% endfor %}
        </ul>
        {% endif %}
        <div><strong>答案：</strong> {{ item.answer | join(", ") }}</div>
    </div>
    {% endfor %}
</body>
</html>
"""
name = '近现代题库'
# 渲染模板
html_output = Template(html_template).render(title=name, data=export_data)

# 保存 HTML 文件
with open(f"{name}.html", "w", encoding="utf-8") as f:
    f.write(html_output)

# PDF导出路径（注意修改路径）
pdfkit_config = pdfkit.configuration(wkhtmltopdf='D:/wkhtmltopdf/bin/wkhtmltopdf.exe')
pdfkit.from_file(f"{name}.html", f"{name}.pdf", configuration=pdfkit_config)

print(f"✅ PDF 导出成功：{name}.pdf")
