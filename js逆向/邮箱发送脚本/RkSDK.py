...
class RkSDK(Singleton):
    ...
    def __GetMacCode(self):
        ...
        for u in cp:
            cpuMd5 = hashlib.md5((u.ProcessorId).encode()).hexdigest()
            break
        return (cpuMd5[11:14] + cpuMd5[7:10] + cpuMd5[3:6] + cpuMd5[(len(cpuMd5) - 3):]).upper()
    ...
    def __heartbeat(self):
        ...
        if self.__isStartHeartBeat == False:
            ...
            while True:
                ...
                RepResult = self.__GetRequestResult(send_data, BusinessType.heartBeat)
                if RepResult.code != 0:
    ...
    def 获取瑞科验证SDK当前版本号(self)->str:
        result= "\n当前瑞科验证SDK版本：v2.3 瑞科官网：http://www.ruikeyz.com  http://rk.ruikeyz.com"
        print(result)
        return result
