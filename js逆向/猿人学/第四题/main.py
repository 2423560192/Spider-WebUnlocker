...
def get_resp(page):
    ...
    response = session.get('https://match.yuanrenxue.cn/api/match/4', params=params)
    print(response.json())
...
def sorted_nums(styles , imgs):
    # 提取所有偏移量，并且 / 11
    styles = [int(float(i.split(':')[1].replace('px' , '')) / 11)  for i in styles]

    print(styles)
    order_nums = [None] * (len(styles))

    for ind , style in enumerate(styles):
        sign = int(ind + style)   # 偏移后的元素位置
        order_nums[sign] = str(nums_dict[imgs[ind]])
...
def parse_data(data):
    res = []
    j_key = md5_encrypt(base64.b64encode(str(data['key'] + data['value']).encode()).decode().replace('=', ''))
    # print(j_key)
    htmls = data['info']
    lxml = etree.HTML(htmls)
    tds = lxml.xpath('//td')
    for td in tds:
        # imgs = td.xpath(f'.//img[not(contains(@class, "{j_key}"))]/@src')
        imgs = td.xpath(f'.//img[not(contains(@class, "{j_key}"))]/@src')
        styles = td.xpath(f'.//img[not(contains(@class, "{j_key}"))]/@style')
        print(styles)

        num = sorted_nums(styles , imgs)
...
if __name__ == '__main__':
    ...
    session.cookies.update({
        ...
        'Hm_lpvt_9bcbda9cbf86757998a2339a0437208e': '1746779176',
        'Hm_lpvt_c99546cf032aaa5a679230de9a95c7db': '1746779179',
    })
    ...
    session.headers = {
        ...
        # 'cookie': 'sessionid=9v4dut5gjjvapw0ox3ls3mp5rt5p3yov; Hm_lvt_434c501fe98c1a8ec74b813751d4e3e3=1745473253,1745670989,1745930833,1746704613; qpfccr=true; Hm_lvt_c99546cf032aaa5a679230de9a95c7db=1746708668,1746756776,1746770541,1746776721; HMACCOUNT=74E03469813A9187; no-alert3=true; Hm_lvt_9bcbda9cbf86757998a2339a0437208e=1746708671,1746756927,1746770631,1746776739; Hm_lpvt_9bcbda9cbf86757998a2339a0437208e=1746779176; Hm_lpvt_c99546cf032aaa5a679230de9a95c7db=1746779179',
    }
