import os
import time
from urllib.parse import urljoin
import pandas as pd
from jsonpath import jsonpath
from db import DB
from concurrent.futures import ThreadPoolExecutor
import requests
from 普通爬虫.防消易平台.kuaidaili import get_ip


def get_img(creditCode, url):
    img_url = urljoin('https://xf.jintongxinxi.com/', url)  # 使用 urljoin 来构建完整的图片 URL
    try:
        response = requests.get(img_url, stream=True)
        response.raise_for_status()  # 确保请求成功
        with open(f'images2/{creditCode}.jpg', 'wb') as f:
            for chunk in response.iter_content(1024):
                f.write(chunk)
        print(creditCode, '图片保存完毕!')
    except requests.exceptions.RequestException as e:
        print(creditCode, f"下载图片失败，错误信息：{e}")


def get_detail(id, url, db):
    get_img(id, url)


def worker(id, url, db):
    # ip = get_ip()  # 获取代理 IP
    db.connect()
    get_detail(id, url, db)  # 获取公司详情
    db.close()


if __name__ == '__main__':
    # 初始化数据库
    db = DB()
    conn, cursor = db.connect()
    db.create_table()
    # 查询数据库表
    insert_query = '''
              SELECT * FROM subsidy_info2;
            '''
    cursor.execute(insert_query)
    tbs = cursor.fetchall()
    db.close()

    lst = []
    p = []  # 临时存储已有的
    for i in tbs:
        if i[3] != '' and i[1] not in p:
            lst.append([i[1], i[3]])
            p.append(i[1])
    print(len(lst))

    res = []
    for i in os.listdir('images2'):
        res.append(i.split('.')[0])

    print(len(res))
    # 使用线程池管理并发任务
    with ThreadPoolExecutor(max_workers=100) as executor:
        # 提交任务到线程池
        for id, url in lst:
            if id in res:
                continue
            executor.submit(worker, id, url, db)
            time.sleep(0.01)  # 每秒 100 次任务，控制提交速率
