...
def main():
    ...
    # 获取今天日期
    today = date.today()  # 当前日期，例如 2025-04-25
    ...
    today_activities = df[
        (df['开始时间'].dt.date == today) | (df['结束时间'].dt.date == today)
    ]
    ...
    print(f"找到 {len(today_activities)} 个今天的活动")
    ...
    # 遍历今天的活动
    for index, row in today_activities.iterrows():
        activity_id = row['ID']
