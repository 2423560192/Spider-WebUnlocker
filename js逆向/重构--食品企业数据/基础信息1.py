...
def start():
    ...
    for i, j in df.iterrows():
        ...
        if '省' in j['region']:
            items['city'] = '-'
            items['prevence'] = j['region']
        else:
            items['city'] = '-'
            items['prevence'] = j['region']
        items['area'] = '-'
        items['state'] = '在营'
        items['category'] = j['quality']
...
def parse_one(url, p, items):
    ...
    # print(proxy)
    try:
        url2 = 'http://127.0.0.1:5555/random'
        ip = requests.get(url2).text
        proxy = {
            'http': f'http://{ip}',
            'https': f'https://{ip}',
        }
        print(proxy)
        response = requests.post(url, headers=h, data=json.dumps(p), timeout=10, verify=False,proxies=proxy)
        time.sleep(1)
...
if __name__ == '__main__':
    ...
    cursor = db.cursor()
    try:
        sql = 'create table 企业基础信息1(id int not null auto_increment primary key ,市级 varchar(255),省级 varchar(255),区级 varchar(255),状态 varchar(255),分类 varchar(255),ids varchar(255),电话 varchar(255),邮箱 varchar(255),企业名称 varchar(255),注册金额 varchar(255),企业简介 varchar(1024),行业大类 varchar(255),行业小类 varchar(255),地址 varchar(255))'
        cursor.execute(sql)
    except Exception as e:
        print('表已存在!', e)
    start()
