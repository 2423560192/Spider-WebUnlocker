...
class Spider:
    ...
    def base_data(self, dic):
        # 爬取行政处罚
        self.p6["params"][0]['value'] = dic['id']
        try:
            self.parse_xingzheng_data(self.url6, self.p6,dic['oid'])
            time.sleep(2)
        except Exception as e:
            print(e)
        # # # 股东信息
        # self.p7["params"][0]['value'] = dic['id']
        # try:
        #     self.parse_gudong_data(self.url7, self.p7)
        # except:
        #     pass
        # # # 判决文书
        # self.p8["params"][0]['value'] = dic['id']
        # self.parse_judge_data(dic['id'], self.infomation_base['统一社会信用代码'])
        # print(self.infomation_gudong)
        # 数据字段
    ...
    # 爬取判决文书
    def parse_judge_data(self, id, user_id):
        url = 'https://holmes.taobao.com/web/detail/companyDetailCardBatch'
        # data={"dataModuleId":456,"params":[{"key":"onecomp_id","value":"1180716000050222313"}],"type":"web","pageNo":1,"pageSize":10}
        data = {"dataModuleIds": [460, 456, 455, 462, 457, 490, 470, 458, 495], "type": "web", "pageNo": 1,
                "pageSize": 10,
                "params": [{"key": "onecomp_id", "value": None}]}
        data['params'][0]['value'] = id
        re = requests.post(url=url, headers=self.h, data=json.dumps(data)).text
        re = json.loads(re)
        try:
            if re['data'][1]['total'] != 0:
                pages = re['data'][1]['total'] // 10 if re['data'][1]['total'] % 10 == 0 else re['data'][1][
                                                                                                  'total'] // 10 + 1
                time.sleep(2)
                re = requests.post(url=url, headers=self.h, data=json.dumps(data)).text
                re = json.loads(re)
                result = re['data'][1]['rows']
                for res in result:
                    r = {}
                    r['统一社会信用代码'] = user_id
                    r['案件名称'] = res[2]
                    ...
                    r['日期'] = res[-1]
                    r['案号'] = res[4]
                    self.infomation_judge.append(r)
    ...
    # 行政处罚
    def parse_xingzheng_data(self, url, p,oid):
    ...
    def parse_gudong_data(self, url, p):
        global res
        ...
        if re2['total'] != 0:
            for i in re2['rows']:
                ...
                result['持股比例'] = i[3]
                result['认缴出资日期'] = i[4]
                self.infomation_gudong.append(result)
    ...
    def save_data(self, num):
        if num == 0:
            ...
            for i in self.infomation_judge:
                self.wb6.append(
                    [str(i['统一社会信用代码']), i['案件名称'], i['案件原由'], i['案件身份']
                        , i['日期'], i['案号']])
                self.ws.save('企业上下游数据.xlsx')
