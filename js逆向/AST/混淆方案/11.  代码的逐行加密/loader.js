...
const {TYPES, blockStatement} = require("@babel/types");
...
const visitor_11 = {
    FunctionExpression(path){
        let {node} = path
        blockStatements = node.body
        let statments = blockStatements.body.map(
            function (item) {
                if (types.isReturnStatement(item)) return item;
                let code = generator(item).code
                // let encCode = b64_encode(code)
                // let atfunc = types.callExpression(types.identifier("atob") , [types.stringLiteral(encCode)])
                // let func = types.callExpression(
                ...
                return types.expressionStatement(func)
            }
        )
        path.get('body').replaceWith(types.blockStatement(statments))
    }
}
traverse(ast, visitor_11)
